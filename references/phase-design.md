...
For each phase, list what the generator must surface into the conversation and what the evaluator will re-run or observe. Behavior-producing phases must include at least one `empirical` item: the running artifact observed, not just "tests pass". That item must be observable **independently of any test or script written in this run**: the evaluator drives the artifact directly (DOM, HTTP response, CLI output, on-disk state). A criterion the evaluator can only confirm by re-running the generator's own test is `tool-output`, not `empirical`; a behavior phase needs at least one criterion beyond it. Common evidence types:
